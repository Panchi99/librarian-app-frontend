                        ...
                        <select name="category" className="form-control" onChange={handleChange}>
                            {props.categories.map((category,index) => {
                                if(props.book.category !== undefined &&
                                    ...
                                    return <option key={index} selected={props.book.category} value={props.book.category.name}>{category}</option>
                                else return <option key={index} value={category.name}>{category}</option>
                            })}
                        </select>
                    </div>
                    <div className="form-group mt-3">
                        ...
                        <select name="authorId" className="form-control" onChange={handleChange}>
                            {props.authors.map((author) => {
                                if(props.book.author !== undefined &&
                                    ...
                                    return <option key={author.id} selected={props.book.author.id} value={author.id}>{author.name}</option>
                                else return <option key={author.id} value={author.id}>{author.name}</option>
                            })}
                        </select>
                    </div>
                    <div className="form-group mt-3">
